...
def getopt(subparsers):
    parser = subparsers.add_parser(name = 'demux',description = 'Make clustre from BAM file that contains alignments with barcode on their ID')
    ...
    parser.add_argument('-c','--conserved', action='store_true', help = 'Use of a voting algorithm to correct read base errors')
...
def run(args):
    ...
    logger.info('Finished clustering: output %i clusters in %.3f min' %(out_count, (time.time() - start) /60))