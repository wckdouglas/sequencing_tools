...
def getopt(subparsers):
    parser = subparsers.add_parser(
        name="filterUMI",
        description="Filter out alignments with UMI having nucleotide runs > 3nt",
    )
    ...
    parser.add_argument(
        ...
        "--outbam",
        default="-",
        help="output interleaved fastq file (defulat: - )",
    )
    parser.add_argument(
        "-c",
        "--consecutive_bases",
        default=3,
        help="Threshold for nucleotide runs (defualt: 3, only output alignments with UMI without AAAA,TTTT,CCCT,GGGG)",
    )
