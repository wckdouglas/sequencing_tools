...
cdef class fragment:
    ...
    def output_fragments(self, int umi_nt = 6, str read_prefix = 'Sample', out_file = sys.stdout):
        '''
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef int parse_dedup(infile, outfile, int umi_nt, str read_prefix):
    cdef:
    ...
    for in_count, line in enumerate(infile):
        ...
        if _fragment.check_fragment(chrom, start, end, strand):
            ...
            if _fragment.chrom != "chrom":
                out_count += _fragment.output_fragments(umi_nt = umi_nt, 
                                                        read_prefix = read_prefix, 
                                                        out_file = outfile)
