...
class Exon:
    ...
    def __init__(self, start: int, end: int, exon_num: int, strand: str) -> None:
        ...
        self.cumulative_transcript_length = (
            None  #: the transcript position of last exon base
        )
...
class Transcript:
    def __init__(self, transcript: Dict[str, Any]) -> None:
        ...
        self.strand = (
            "+" if transcript["strand"] in ["+", 1] else "-"
        )  #: strand ('+' or '-')
        self.tx_start = int(
            transcript["tx_start"]
        ...
        self.exon_starts = transcript["exon_starts"].split(",")[
            :-1
        ]  #: str, comma list of exon start sites
        self.exon_ends = transcript["exon_ends"].split(",")[
            :-1
        ]  #: str, comman list of exon end sites
        self.five_UTR_length = (
            self.tx_start - self.cds
    ...
    def __MakeTranscripts__(self) -> None:
        ...
        for exon_num in range(self.exon_count):
            ...
            if exon.start <= self.cds <= exon.end:
                after_cds += 1
                exon.contain_cds = 1
                distance_from_exon_start = (
                    self.cds - exon.start if exon.strand == "+" else exon.end - self.cds
                )
                self.cds_off_set = (
                    distance_from_exon_start + cumulative_transcript_length
                )

            if exon.start <= self.cde <= exon.end:
                after_cde += 1
                exon.contain_cde = 1
                distance_from_exon_end = (
                    self.cde - exon.start if exon.strand == "-" else exon.end - self.cde
                )
            ...
            if exon.after_cds:
                if not exon.contain_cds and not exon.contain_cde:
                    ...
                    """
                    exon.coding_bases = (
                        exon.length - distance_from_exon_end - distance_from_exon_start
                    )
...
class Transcriptome:
    ...
    def __init__(self, refflat: str=None, sqldb: str=None, coding_only: bool=True) -> None:
        ...
        )  #: Dict storing transcripts, with gene name as key and values are :class:`sequencing_tools.gene_tools.transcriptome.Transcript`
        ...
        if sqldb:
            ...
            self.url = (
                refflat
                or "http://hgdownload.soe.ucsc.edu/goldenPath/hg19/database/refFlat.txt.gz"
            )
        ...
        for tab in refflat:
            for i, transcript in tab.iterrows():
                ...
                if transcript["cds"] != transcript["cde"] or not self.coding_only:
                    self.transcript_dict[transcript["Gene name"]][
                        transcript["tid"]
    ...
    def __MakeTranscriptomeFromSqlite(self) -> None:
        ...
        """
        transcriptDB = (
            read_sql_query(query, con=self.sql_connection)
            .groupby(
                [
                    "tx_id",
                    "tx_biotype",
            ...
            .agg(
                {
                    "exon_seq_start": lambda xs: ",".join(map(str, xs)) + ",",
        ...
        for i, transcript in transcriptDB.iterrows():
            if transcript["tx_biotype"] == "protein_coding" or not self.coding_only:
                self.transcript_dict[transcript["gene_name"]][
                    transcript["tid"]
                ] = Transcript(transcript)
