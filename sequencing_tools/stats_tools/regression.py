...
class GradientDescent:
    ...
    def Cost(self):
        ...
        for i in range(self.n_coefficients):
            self.gradient[i] = self.summary(
                -self.X[:, i] * self.residuals[i]
            )  # graident for each coefficient
            self.gradients[self._iter - 1, i] = self.gradient[
                i
            ]  # store gradient of the coefficient
    ...
    def Adam_update(self):
        ...
        for i in range(self.n_coefficients):
            self.moving_average_gradient = (
                self.beta_1 * self.moving_average_gradient
                + (1 - self.beta_1) * self.gradient[i]
            )
            self.moving_average_squared_gradient = (
                self.beta_2 * self.moving_average_squared_gradient
                + (1 - self.beta_2) * (self.gradient[i] * self.gradient[i])
            )
            m_cap = self.moving_average_gradient / (
                1 - (self.beta_1 ** self._iter)
            )  # calculates the bias-corrected estimates
            v_cap = self.moving_average_squared_gradient / (
                1 - (self.beta_2 ** self._iter)
            )  # calculates the bias-corrected estimates

            self.diff = (
                self.learning_rate * m_cap / (np.sqrt(v_cap) + self.epsilon)
            )  # Adam optimize
            self.diffs[
                i
            ] = (
                self.diff
            )  # store diff for all coeficients in one array, for estimating whether coefficents are changing in this iteration
    ...
    def fit(self, X, y):
        ...
        self.B_history = np.zeros(
            (self.max_iter, self.n_coefficients)
        )  # history of optimized coefficients
        ...
        self.bootstrap_idx = self.bootstrap.generate(
            X, group_size=len(X) // 10, n_boots=int(self.max_iter * 2.5)
        )
        ...
        while (
            self._iter < self.max_iter and self.iter_no_change < self.n_iter_no_change
        ):
            self._fit()
        ...
        if self.iter_no_change < self.n_iter_no_change:
            ...
            self.converge = True
            message = "Converged at the %ith iteration: Cost %.3f" % (
                self._iter,
                self.cost,
            )
        if self.verbose:
