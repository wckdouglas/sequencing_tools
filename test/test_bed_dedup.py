...
def test_fragment_group():
    ...
    for coordinates, fragments in groupby(expected, bed_dedup.fragment_coordinates):
        barcode_group = bed_dedup.fragment_group(
    ...
    assert uc == 2
    assert set(barcode_group.unique_barcodes).issubset(
        ["ACT_4_members", "ACG_4_members", "GGG_2_members"]
    )

    max_member_count = barcode_group.demultiplexing_barcodes(threshold=0)
    assert max_member_count == 3
    assert set(barcode_group.unique_barcodes) == set(
        ["ACG_1_members", "GGG_2_members", "ACT_3_members"]
    )
